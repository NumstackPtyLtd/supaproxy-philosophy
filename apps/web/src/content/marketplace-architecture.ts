...
export const article: Article = {
  ...
  blocks: [
    ...
    { type: 'heading', text: 'Guardrails already exist. Infrastructure does not.' },
    { type: 'paragraph', text: 'Here is the thing we keep coming back to. Guardrails are not a new problem. NVIDIA built NeMo Guardrails for conversational AI safety. Guardrails AI built a validation framework for LLM outputs. Lakera built prompt injection detection. Rebuff built LLM output filtering. These are serious tools built by teams who understand the problem deeply.' },
    { type: 'paragraph', text: 'We are not going to build a better PII detector than a team that has spent three years on nothing else. We are not going to build a better prompt injection detector than a security company that specialises in adversarial AI. And we should not try.' },
    { type: 'paragraph', text: 'What does not exist is the infrastructure to compose these tools. An organisation that wants NeMo-style rails on their conversational AI, plus Lakera-style injection detection, plus their own custom compliance rules cannot do that today without building the plumbing themselves. They need to figure out the execution order, the data flow between guardrails, the per-workspace configuration, the audit trail, the override policies.' },
    { type: 'paragraph', text: 'That is what we are building. Not guardrails. Guardrail infrastructure. The marketplace is how the best guardrails in the ecosystem get composed, configured, and governed on the infrastructure that handles everything else.' },
    { type: 'callout', variant: 'insight', title: 'The real question', text: 'The question is not what guardrails to build. It is how to let organisations compose the guardrails that already exist, alongside the ones they build themselves, on infrastructure that handles the execution, audit, and governance.' },

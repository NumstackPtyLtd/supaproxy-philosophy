...
export const article: Article = {
  ...
  subtitle: 'Not a chatbot. Not a router. A governed mediator that holds context, enforces policy, and never pretends to know more than it does. This is the architecture we are building.',
  ...
  featured: true,
  blocks: [
    { type: 'heading', text: 'What concerns us about AI customer service' },
    { type: 'paragraph', text: 'The default approach to AI customer service is straightforward. Put a chatbot on the website, connect it to some APIs, let it answer questions. Without thinking it through, this is probably where we would have started too. It works for demos. It falls apart in production.' },
    ...
    { type: 'paragraph', text: 'But memory without trust is dangerous. We covered this extensively in [Your AI product has amnesia. Here is how to fix it](/memory-and-trust). The trust layer is the policy check that happens before every action. Before the receptionist routes to a workspace, it verifies whether the workspace exists, whether the org policy allows this routing, and whether the confidence score is high enough to route automatically or whether it should ask the customer. Every decision gets a confidence score, and low-confidence decisions get turned into questions, not guesses.' },
    { type: 'paragraph', text: 'This is where the guardrails come in. Not as classifiers that pass or block, but as [middleware that transforms](/guardrail-middleware). A guardrail that detects the customer is asking for financial advice does not just block the response. It transforms it into a referral, something like "I can ask a licensed advisor to get back to you. Would you like that?" A guardrail that detects PII does not just redact it. It checks the org policy to determine whether the PII is needed for customer identification. If yes, use it for routing. If no, handle according to platform defaults.' },

    { type: 'heading', text: 'The personality problem' },
    { type: 'paragraph', text: 'The tempting approach to personality is a system prompt. You write a paragraph about tone and hope the model follows it. This breaks in production because the personality competes with every other instruction in the prompt for the model\'s attention.' },
    ...
    { type: 'paragraph', text: 'The conversation does not end because the model runs out of things to say. It ends because the system tool explicitly closes it. After three turns on an issue without resolution, the receptionist wraps up with "Is there anything else I can help with?" If the customer says no, the receptionist calls close_conversation, which triggers the analysis pipeline. If the customer spirals after a ticket has been logged, the receptionist closes politely, confirming the ticket is noted and thanking the customer.' },
    ...
    { type: 'callout', variant: 'warning', title: 'The leaking problem', text: 'One of the risks we think about constantly is internal leakage under pressure. A customer asks a difficult question, the model panics, and suddenly the response contains tool names, error codes, or API references. The receptionist guardrails are specifically designed to catch and transform these before they reach the customer.' },

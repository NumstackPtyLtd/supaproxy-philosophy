...
export const article: Article = {
  ...
  blocks: [
    ...
    { type: 'paragraph', text: 'Not all meaning needs to be preserved. The question is what the AI needs to reason correctly, and what can safely change.' },
    ...
    { type: 'paragraph', text: 'A privacy layer that claims to be enterprise-ready cannot ship with a fixed set of format rules. It must be configurable per organisation, per domain, per entity type. The platform provides the mechanisms. The organisation provides the policy.' },
    ...
    { type: 'paragraph', text: 'The right approach is to give organisations a framework where they can define, per entity type, exactly which semantic dimensions to preserve and which to perturb. The framework validates that the rules are internally consistent (you cannot preserve age bracket if you randomize the DOB field entirely). It audits that the rules meet the regulatory requirements the organisation claims to satisfy. And it generates encrypted avatars that are both privacy-safe and semantically useful for the specific AI workload they serve.' },

    { type: 'callout', variant: 'warning', title: 'The risk of false confidence', text: 'The most dangerous outcome is an organisation that deploys format-preserving encryption, assumes the AI is reasoning correctly, and does not realize the encrypted data broke a semantic assumption the model depends on. The ID looks valid. The model trusts it. The conclusion is wrong. And no one catches it because the output looks plausible. This is why semantic preservation must be an explicit, auditable configuration, not an implicit side effect of the encryption algorithm.' },

    { type: 'heading', text: 'Building for the spectrum' },
    { type: 'paragraph', text: 'The practical path forward is a layered system. At the base layer, structural format preservation ensures a name becomes a name and a number becomes a number. This is table stakes and should work out of the box. Above that, semantic preservation rules that organisations configure per entity type and per domain. These are the rules that say "preserve the age bracket in SA IDs" or "derive the email from the avatar name." At the top, cross-entity consistency that ensures the fictional identity holds together under cross-reference.' },
    { type: 'paragraph', text: 'Each layer adds complexity and configuration cost. Not every organisation needs all three. A customer support chatbot may only need structural preservation. A credit scoring engine needs all three. The platform must support the full spectrum without forcing the full cost on every deployment.' },
  ...
  validate?(value: string): boolean
    ...
    { type: 'heading', text: 'Why custom packages are a self-hosted capability' },
    { type: 'paragraph', text: 'Custom encryption packages run at the most sensitive point in the pipeline. They are the last code that touches real data before it becomes protected. That code must be trusted completely. On self-hosted infrastructure, trust is simple. The organisation wrote the package, deployed it, and controls the machine it runs on. The data never leaves their network.' },
    { type: 'paragraph', text: 'Running custom encryption packages in a managed cloud is a different problem entirely. The package sees real data. It has to. That is what it encrypts. Sandboxing, audit logging, code signing, source review. All solvable, but each adds complexity, attack surface, and a trust relationship that enterprises in regulated industries may not accept. The compliance team at a bank is not going to approve uploading their ID encryption logic to a third-party cloud, no matter how well sandboxed it is.' },
    { type: 'paragraph', text: 'So custom packages are a self-hosted capability. That is not a limitation. It is a feature. Organisations that need demographic-aware ID encryption or jurisdiction-preserving account numbers are exactly the organisations that should be running their own infrastructure. The sensitivity of their data is why they need custom encryption in the first place. It is also why they should control the machine it runs on.' },
    { type: 'paragraph', text: 'Cloud users get the platform defaults. Names become names, numbers preserve digit count, emails keep valid structure, phone numbers keep their format. These defaults are good enough for most use cases. When they are not, the answer is not to add complexity to the cloud. The answer is to self-host. Custom encryption packages become a documented, concrete reason to move to self-hosted, not a gap in the cloud offering.' },
    { type: 'paragraph', text: 'For enterprise cloud users who need something specific, the platform team can ship it on request as a first-party encryptor. The interface is the same. The package just lives in the platform rather than in the customer\'s infrastructure. This keeps the cloud simple, keeps the trust model clean, and gives the enterprise exactly what they need without building a package runtime in the cloud.' },
  ],
}
